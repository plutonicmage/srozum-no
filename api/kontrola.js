...
// Jednoduchý denní limit přes cookie (výchozí stav).
// TODO(LIMIT): pro ostrý růst zapni serverovou evidenci — Vercel KV/Upstash:
//   1) Vercel → Storage → Create KV → propojit s projektem (env se doplní samy)
//   2) npm balíček není potřeba — použij REST: fetch(process.env.KV_REST_API_URL...)
//   3) Klíč `limit:${ip}:${den}` s TTL 86400; při existenci vrať 429.
...
// Ověření Plus session. Cookie sz_plus nastavuje /api/platby po platbě.
// TODO(PLATBY): po napojení plateb ověřuj podpis tokenu (HMAC se
// SECRET z env) — viz api/platby.js, funkce vytvorPlusToken/overPlusToken.
function jePlus(req) {
  const cookie = req.headers.cookie || "";
  return /sz_plus=/.test(cookie);
}
...
export default async function handler(req, res) {
  ...
  if (!jePlus(req) && checkDailyLimit(req)) {
